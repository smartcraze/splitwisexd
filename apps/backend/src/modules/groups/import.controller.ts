...
const parseDateStr = (
  dateStr: string,
): { date: Date; anomaly: boolean; message?: string } => {
...
export const parseCSVImport = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const resolvedRows = rawRecords.map((raw: any, idx: number) => {
      ...
      if (matchedPayer) {
        ...
        if (
          matchedPayer.user.name.toLowerCase() !==
          rawPaidBy.trim().toLowerCase()
        ) {
          anomalies.push({
            id: 8,
      ...
      const isSettlement =
        isSettlementKeyword ||
        (!rawSplitType &&
          splitWithParticipants.length === 1 &&
          !rawSplitDetails);
      ...
      for (const pName of participantsList) {
        ...
        if (
          mem.user.name.toLowerCase() === "meera" &&
      ...
      if (activeMemberCount > 0 && !skipped) {
        if (splitMethod === "EQUAL") {
          ...
          const detailPairs = splitDetails
            .split(";")
            .map((p: string) => p.trim())
            .filter((p: string) => p.length > 0);
          ...
          if (splitMethod === "UNEQUAL") {
            ...
            if (Math.abs(sumDetails - 100) > 0.01) {
              ...
              if (remainder !== 0 && participants[0]) {
                participants[0].owedAmount =
                  (participants[0].owedAmount || 0) + remainder;
              }
            }
      ...
      if (!skipped && paidByUserId) {
        const isDuplicateInDB = existingExpenses.some((exp) => {
          ...
          const sameTitle =
            ...
            exp.title.toLowerCase().replace(/[^a-z0-9]/g, "") ===
        ...
        if (isDuplicateInDB) {
          anomalies.push({
            ...
            message:
              "Potential duplicate: An identical expense already exists in this group.",
            severity: "warning",
          });
        }
      }
...
export const commitCSVImport = asyncHandler(
  async (req: Request, res: Response) => {
    const authReq = req as AuthRequest;
    if (!authReq.user) {
      throw new AppError("Not authenticated", 401);
    ...
    if (expenses && Array.isArray(expenses)) {
      for (const e of expenses) {
        if (!memberSet.has(e.paidByUserId)) {
          throw new AppError(
            `Payer ${e.paidByUserId} is not a member of this group.`,
            400,
          );
        }
        if (
          ...
          throw new AppError(
            `Expense "${e.title}" must have at least one participant.`,
            400,
          );
        }
    ...
    if (settlements && Array.isArray(settlements)) {
      for (const s of settlements) {
        if (!memberSet.has(s.paidByUserId)) {
          throw new AppError(
            `Settlement sender ${s.paidByUserId} is not a member of this group.`,
            400,
          );
        }
        if (!memberSet.has(s.paidToUserId)) {
          throw new AppError(
            `Settlement receiver ${s.paidToUserId} is not a member of this group.`,
            400,
          );
        }
      }
    ...
    res
      .status(201)
      .json(ApiResponse.success("Data imported successfully", result));
  },
);
