...
export function CommentsSection({ expenseId, currentUserId }: { expenseId: string; currentUserId: string }) {
  ...
  return (
    <div className="flex flex-col h-[400px] border border-border bg-card rounded-xl shadow-sm overflow-hidden">
      <div className="bg-muted px-4 py-3 border-b border-border flex items-center justify-between text-xs font-semibold">
        ...
        <span className={connected ? "text-emerald-500 font-medium" : "text-amber-500 font-medium"}>
          ● {connected ? "Connected" : "Reconnecting"}
        </span>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {comments.length === 0 ? (
          <div className="text-center text-xs text-muted-foreground pt-10">No comments yet. Start the discussion!</div>
        ) : (
          comments.map((comment) => {
            const isSelf = comment.userId === currentUserId;
            return (
              <div key={comment.id} className={`flex items-start gap-2.5 ${isSelf ? "flex-row-reverse" : ""}`}>
                <Avatar className="h-7 w-7 bg-secondary border border-border">
                  <AvatarFallback className="text-[10px] font-bold">{getInitials(comment.user.name)}</AvatarFallback>
                </Avatar>
                <div className={`max-w-[75%] rounded-lg p-2.5 text-sm ${isSelf ? "bg-primary text-primary-foreground" : "bg-muted text-foreground"}`}>
                  {!isSelf && <div className="text-[10px] font-bold text-muted-foreground mb-0.5">{comment.user.name}</div>}
                  <p className="leading-normal break-words">{comment.message}</p>
      ...
      <form onSubmit={handleSubmit} className="p-3 border-t border-border bg-card flex gap-2">
        <Input placeholder="Say something..." value={newComment} onChange={(e) => setNewComment(e.target.value)} className="bg-background border-border text-sm flex-1 focus:ring-ring" />
