...
function DonutChart() {
  ...
  const segments = CATEGORIES.map((cat) => {
    ...
    const seg = { ...cat, dashOffset: -offset, dashArray: `${dash} ${circumference - dash}` };
    offset += dash;
    return seg;
  });

  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-[120px] h-[120px] -rotate-90">
      ...
      <div className="flex items-center gap-5">
        ...
        <div className="relative shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-sm font-extrabold leading-tight">{fmt(totalAmount)}</span>
          </div>
        </div>
